...
RegisterServerEvent("gd-piggyback:sync")
AddEventHandler("gd-piggyback:sync", function(targetSrc)
	local source = source
	...
	if #(sourceCoords - targetCoords) <= 3.0 then 
		TriggerClientEvent("gd-piggyback:syncTarget", targetSrc, source)
		piggybacking[source] = targetSrc
		beingPiggybacked[targetSrc] = source
	end
end)

RegisterServerEvent("gd-piggyback:stop")
AddEventHandler("gd-piggyback:stop", function(targetSrc)
	local source = source

	if piggybacking[source] then
		TriggerClientEvent("gd-piggyback:cl_stop", targetSrc)
		piggybacking[source] = nil
		beingPiggybacked[targetSrc] = nil
	elseif beingPiggybacked[source] then
		TriggerClientEvent("gd-piggyback:cl_stop", beingPiggybacked[source])
...
AddEventHandler('playerDropped', function(reason)
	...
	if piggybacking[source] then
		TriggerClientEvent("gd-piggyback:cl_stop", piggybacking[source])
